...
运行结果 ：default message [不是一个合法的电子邮件地址];**使用数据校验，可以保证数据的正确性；**
...
**注意：如果yml和properties同时都配置了端口，并且没有激活其他环境 ， 默认会使用properties配置文件的！**配置文件加载位置
...
2、新建一个普通Maven模块：shawn-spring-boot-starter

3、新建一个Springboot模块：shawn-spring-boot-starter-autoconfigure
